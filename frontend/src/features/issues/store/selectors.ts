...
import { matchesEmployee, matchesSearch, matchesStatus } from "@/features/issues/utils/filters";
...
export const selectEmployeeStats = (state: Pick<IssuesStoreState, "issues">) => {
...
export const selectIssueById = (state: IssuesStoreState, issueId: string): Issue | undefined =>
	state.issues.find((issue) => issue.id === issueId);

export const selectSelectedCount = (state: { selectedIssues: Set<string> }): number =>
	state.selectedIssues.size;

export const selectHasSelection = (state: { selectedIssues: Set<string> }): boolean =>
	state.selectedIssues.size > 0;
...
export const selectTotalIssuesCount = (state: Pick<IssuesStoreState, "issues">): number =>
	state.issues.length;

export const selectIssuesByIds = (state: Pick<IssuesStoreState, "issues">, issueIds: string[]) =>

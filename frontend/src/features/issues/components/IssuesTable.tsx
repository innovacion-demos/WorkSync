...
export function IssuesTable() {
	const { selectedIssues, toggleIssueSelection, selectAllIssues, clearSelection, getFilteredIssues } = useIssuesStore();
	const filteredIssues = getFilteredIssues();

	const allSelected = filteredIssues.length > 0 && filteredIssues.every((issue) => selectedIssues.has(issue.id));
	const someSelected = filteredIssues.some((issue) => selectedIssues.has(issue.id)) && !allSelected;
								...
								>
									...
									<td className="px-6 py-4 whitespace-nowrap">
										<div className="flex items-center">
											...
											<span className="text-sm text-gray-900">{issue.requester}</span>
										</div>
									</td>
									<td className="px-6 py-4 whitespace-nowrap">
										<span
									...
									<td className="px-6 py-4 whitespace-nowrap">
										<span className={`text-sm flex items-center gap-1 ${priorityStyles[issue.priority]}`}>
											<span className="text-base">{priorityIcons[issue.priority]}</span>

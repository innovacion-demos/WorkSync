...
export const issuesApi = {
	...
	async fetchAll(): Promise<Issue[]> {
		try {
			const response = await fetch(
				`${API_CONFIG.BASE_URL}${ENDPOINTS.ISSUES.BASE}`,
			);
			if (!response.ok)
	...
	async assign(issueId: number, userId: number): Promise<Issue | null> {
		try {
			...
			const response = await fetch(
				`${API_CONFIG.BASE_URL}${ENDPOINTS.ISSUES.ASSIGN(issueId)}`,
				{
					method: "PUT",
					headers: { "Content-Type": "application/json" },
	...
	async updateStatus(
		...
		try {
			...
			const response = await fetch(
				`${API_CONFIG.BASE_URL}${ENDPOINTS.ISSUES.STATUS(issueId)}`,
				{
					method: "PUT",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify(body),
			...
			console.error(
				`[IssuesAPI] Failed to update status for issue ${issueId}:`,
				error,
			);
			return null;
		}
	...
	async unassign(issueId: number, userId = 0): Promise<Issue | null> {
		try {
			...
			const response = await fetch(
				`${API_CONFIG.BASE_URL}${ENDPOINTS.ISSUES.UNASSIGN(issueId)}`,
				{
					method: "PUT",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify(body),
